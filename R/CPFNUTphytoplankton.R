#' @title Phytoplankon data from the chlorpyrifos-nutrient (CPF-NUT) experiment
#' from van den Brink & ter Braak (1998)
#'
#' @description The dataframe \code{CPTNUT_phytoplankton} contains the design
...
#' treatments: control (\code{con}), nutrients (\code{nut}) and nutrients with
#' chlorpyrifos application (\code{cpf-nut}), each with
...
#' @format A data frame with 156 rows and 31 columns:
#'
#' \itemize{
#' \item\code{samples} character identifier for the sample.
#' \item\code{Time} factor coding for the week of sampling with respect to first
#'  application of nutrients.
#' \item\code{Treatment}  treatment factor as in
#' van den Brink & ter Braak (1998).
#' The levels are \code{con}, \code{nut} and \code{cpf-nut}
...
#' \item cell counts of 25 different phytoplankton taxa in 1L,
#' and density for the 26th species, Volvox.
#' For details, see van den Donk et al. (1995).
#' }
#' @source  van Donk et al. (1995), van den Brink & ter Braak (1998)
#' @references
...
#' Aquatic ecology, 32, 163-178. \doi{10.1023/A:1009944004756}
#'
...
#' Hydrobiol 133: 417–439. \doi{10.1127/archiv-hydrobiol/133/1995/417}
#'
#' @name CPFNUTphytoplankton
#' @keywords datasets
#' @docType data
